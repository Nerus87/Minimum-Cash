...
stock void SetCvarsOnStart() 
{
	...
	*/
	 ...
	 * Register ConVars
	*/
	CreateConVar("sm_mincash_version", PLUGIN_VERSION, PLUGIN_DESCRIPTION, FCVAR_PLUGIN | FCVAR_SPONLY | FCVAR_NOT_CONNECTED);

	sm_mincash_enabled = CreateConVar("sm_mincash_enabled", "1", "Enable or disable mincash plugin: 0 - disabled, 1 - enabled", FCVAR_PLUGIN | FCVAR_SPONLY | FCVAR_NOT_CONNECTED);
	
	sm_mincash_cash = CreateConVar("sm_mincash_cash", "0", "Minimum player cash on spawn, if 0 value will be same like mp_startmoney", FCVAR_PLUGIN | FCVAR_SPONLY | FCVAR_REPLICATED | FCVAR_NOT_CONNECTED);
